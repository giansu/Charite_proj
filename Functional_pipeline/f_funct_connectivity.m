function [PLV_matrix, AEC_matrix] = f_funct_connectivity (signal, signal_filt_phases, signal_filt_amplitudes, good_samples, edges)
...
% to obtain FC for DMN signal
    ...
    for i_ROI_riga = 1:size(signal,1)
        for i_ROI_col = 1:size(signal,1)
            for t = good_samples
                p_diff_delta (i_ROI_riga,i_ROI_col,(t-edges)) = single( exp ( (i* (signal_filt_phases (i_ROI_riga,t,1)-signal_filt_phases (i_ROI_col,t,1)) ) ) );
                p_diff_theta (i_ROI_riga,i_ROI_col,(t-edges))= single( exp ( (i* (signal_filt_phases (i_ROI_riga,t,2)-signal_filt_phases (i_ROI_col,t,2)) ) ) );
                p_diff_alpha (i_ROI_riga,i_ROI_col,(t-edges))= single( exp ( (i* (signal_filt_phases (i_ROI_riga,t,3)-signal_filt_phases (i_ROI_col,t,3)) ) ) );
                p_diff_beta (i_ROI_riga,i_ROI_col,(t-edges))= single( exp ( (i* (signal_filt_phases (i_ROI_riga,t,4)-signal_filt_phases (i_ROI_col,t,4)) ) ) );
                p_diff_gamma (i_ROI_riga,i_ROI_col,(t-edges))= single( exp ( (i* (signal_filt_phases (i_ROI_riga,t,5)-signal_filt_phases (i_ROI_col,t,5)) ) ) );
            end
